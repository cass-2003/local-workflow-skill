...
|---|---|---|
| AWS access key | `AKIA...` with the full provider-specific length | high |
...
| Private key block | `<BEGIN RSA PRIVATE KEY BLOCK>` | high |
| JWT | `eyJ...` three-part | high |
| Env-style secret assignment | `<PASSWORD>=<value>` | high |
...
| URL with token query param | `https://api/?<token-param>=...` | high |
...
```
Example key: <AWS_ACCESS_KEY_EXAMPLE> <!-- handoff:allow secret -->
```
