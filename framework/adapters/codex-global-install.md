...
2. 声明持续推进时先做项目规划，再按自主循环实现、验证、自审、修复、同步和提交
3. 指向这套框架的真实核心位置
...
- 审计、验收预检、重要 review 默认留下 `docs/audit/` 报告并更新索引
- 实现、修复、验证和文档同步默认回写日志、进度、文档入口或相关目录索引
- 用户要求继续推进项目时，先检查或生成路线图和下一步工作包，再进入 `08-autonomous-project-loop.md`
...
- 审计、验收、实现、修复和文档同步不会只停留在聊天里；会按 `07-artifact-contracts.md` 留下可发现产物
- “继续推进项目 / 自动审计修复”会触发初始规划与自主循环，而不是完成一个局部动作后无理由停下
