...
记录 `skills/` 从 42 → 409 → 439 → 517 → 523 → 557 技能是怎么合并和扩容出来的。脚本默认使用仓库相对路径和 `~/.codex/skills`，也可以通过环境变量改写来源位置，保留是为了**可复现与可审计**。
...
体检基线：557 赢家 · frontmatter 0 缺失 · 0 空目录 · 0 跨域重复 slug · README 计数 0 不符。
